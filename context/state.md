...
**Evidence.** Integrator's independent targeted rerun on the stacked tree: Ran 30 tests in 47.098 s, OK. Full suite on
the stacked tip `e6dedfb` (`/var/home/omid/.openclaw/workspace/scratch/nb-lanes/run/integ-auto-0925-1835.log`, started 6:36 PM): **Ran 2067 tests in 2044.084 s, OK (skipped=1), exit 0**.

**What landed.**

- **Lane 2 (2D parity), step V3 of 3: region of interest, proxy toggle and format masks.** Commits:
  - `1c4d084` Viewer: region of interest that clips the tile request, saved proxy tier with a 1/8 step and on-screen badge, and display-only format masks (1.33 to 2.40, lines/half/full)
  Diff: 10 files changed, 795 insertions(+), 7 deletions(-).
  Plain description: the lane's report file under /tmp/nb-auto and issue #2.
...
**Evidence.** Integrator's independent targeted rerun on the stacked tree: Ran 34 tests in 21.430 s, OK. Full suite on
the stacked tip `7f3dd97` (`/var/home/omid/.openclaw/workspace/scratch/nb-lanes/run/integ-auto-0925-1005.log`, started 10:05 AM): **Ran 1946 tests in 1246.811 s, OK (skipped=1), exit 0**.

**What landed.**

- **Lane 4 (rendering), step C of 4: kept specular.** Commits:
  - `90be21f` Keep the capture's specular through splat relighting and shadow catching (Keep specular knob)
  Diff: 10 files changed, 304 insertions(+), 11 deletions(-).
  Plain description: the lane's report file under /tmp/nb-auto and issue #4.

Limits: Linux only (RTX 3080 Ti); no Windows run; CI on the pushed commit not read; visual QA on the
real display owed by Gonzo. Lane-reported limits are in each lane's report file and issue.

## Continuous mode merge: Lane 4 (rendering) (9:55 AM on 2026-09-25 PDT)

`main` moved `947338c` -> `1883fff` (lane commits cherry-picked onto main in lane order) and then to this
docs commit, by the continuous-lane integrator tick (`scratch/nb-lanes/auto/tick.py` in Gonzo's
workspace; mode approved by DiMo on 2026-09-23 at 2:39 PM PDT).

**Evidence.** Integrator's independent targeted rerun on the stacked tree: Ran 33 tests in 23.070 s, OK. Full suite on
the stacked tip `1883fff` (`/var/home/omid/.openclaw/workspace/scratch/nb-lanes/run/integ-auto-0925-0925.log`, started 9:25 AM): **Ran 1937 tests in 1242.257 s, OK (skipped=1), exit 0**.

**What landed.**

- **Lane 4 (rendering), step B of 4: shadow offset and blur controls.** Commits:
  - `c248946` GPU shadow bias, blur and samples in the raster and ray-traced paths
  - `63f26cb` Light3D shadow bias, blur and samples on the CPU reference
  Diff: 10 files changed, 482 insertions(+), 51 deletions(-).
  Plain description: the lane's report file under /tmp/nb-auto and issue #4.
...
**Evidence.** Integrator's independent targeted rerun on the stacked tree: Ran 100 tests in 16.946 s, OK. Full suite on
the stacked tip `777d3de` (`/var/home/omid/.openclaw/workspace/scratch/nb-lanes/run/integ-auto-0924-2155.log`, started 9:55 PM): **Ran 1910 tests in 1244.507 s, OK (skipped=1), exit 0**.

**What landed.**

- **Lane 2 (2D parity), step 4c of 3: shared format registry, Grid, NoOp.** Commits:
  - `286a965` L2 step 4c: document-wide format registry, Grid, NoOp
  Diff: 14 files changed, 520 insertions(+), 27 deletions(-).
  Plain description: the lane's report file under /tmp/nb-auto and issue #2.
