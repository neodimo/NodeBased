...
Status: contract written 2026-09-10, before implementation, on branch
`spike/roto-tracker`. Judged the way `docs/PLAYBACK.md` and
`docs/EVALUATION_TIERS.md` were judged — the contract is fixed first, then the
...
and resolution matches `nodebased.animation.resolve_params` clause for clause:

* before the first key or after the last key — `value` applies, which is why the
  static form is just the number;
* at an exact key frame — that key's value;
* between keys — `linear` interpolates, `constant` holds the previous key.

The pairing of a base `value` with an optional `curve` is deliberate: v6 keeps
the static value on the node and the keys in a separate section, and
out-of-range resolution falls back to the static value. An inline curve with no
...
`nodebased.tracker.analyse` is the thing that produces track keys from pixels:
zero-mean normalised cross-correlation of a pattern window against a search
window, integer peak, then a parabolic sub-pixel refinement on the correlation
surface in x and y independently. It walks frames in order, seeding each frame's
search from the previous frame's result.

It is a pure function of the images it is given, so it is deterministic and
testable, but it is **library surface only** in this pass: no UI tracking
button, no agent op, no in-viewer track markers. Wiring it to the interface is
named work for the next owner, not something this branch claims.
