...
Not built: GPU ray tracing of splat scenes (mesh/splat mutual shadowing on the GPU), GPU splat shadows and transparent-mesh layering,
...
desktop app still has to consume the callback): the rest of the GPU ray-traced mode (mesh beauty shading and all mesh AOV outputs have landed; splat casters remain) designed for mesh/splat mutual shadowing (banded GPU submissions with cancel points have
