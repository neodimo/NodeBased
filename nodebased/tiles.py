...
SUPPORTED_TILED_KINDS = frozenset({
    ...
    "Erode", "Dilate", "Median", "Sharpen", "Glow", "Soften", "Defocus", "DirBlur",  # halo = (size, size), same padded-filter shape as Blur
    ...
    # coordinate-dependent exactly like Transform's affine one. Both fall back to the full-frame
    # evaluator, asserted equal to it.
})
...
DEFAULT_HALO_PER_KIND = {
    ...
    "Erode": (0, 0), "Dilate": (0, 0), "Median": (0, 0), "Sharpen": (0, 0), "Glow": (0, 0), "Soften": (0, 0), "Defocus": (0, 0), "DirBlur": (0, 0),
...
def resolve_halo(kind: str, params: dict | None) -> tuple:
    ...
    *kernel's own output space*, which is also its tile space at any given tier; tier scaling of
    ...
    if kind == "DirBlur":
        import math
        # Mirror `tiers._dirblur_rule` for the linear type, the only one on the tile path.
