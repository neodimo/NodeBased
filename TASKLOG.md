## 2026-09-14 — Closeout audit at shipped v0.15.0 follow-up
...
- **Scope/evidence:** Audited exact commit `52bb039f20b6871179ba02081a520bf0ddd8257e`,
  ...
  in `context/state.md`, `context/HANDOFF.md`, and `docs/RELEASE_NOTES.md`.
...
- **Verification:** Focused Roto suite passed **48 tests in 0.844s**;
  full offscreen discovery passed **484 tests in 146.851s**. Qt emitted only
  the known `propagateSizeHints()` offscreen warnings.
- **Additional exact-head evidence:** Supplied Xvfb validation on `main` at
  `52bb039` passed decoder frames **1, 3, 7, 11, 12**. `qa_exr_playback.py`
  passed with 512px/no blur at **12/12 displayed, 24.0 fps, 0.08s maximum
  no-update gap**, and 1600px/Blur at **12/12 displayed, 18.2 fps, 0.34s
  maximum gap**; both verdicts were `playing`. Evidence is local under
  `scratch/closeout-exrqa` and is not a committed artifact. This is Xvfb
  evidence; native-display QA remains unverified.
- **State:** Durable closeout only. No product implementation, tag, release, or push
